...
from img2_game_generator import create_board_image
from data.img2.prepare_img2 import load_and_process_image
...
def generate_graphs(all_results):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('assets/img2_accuracy_across_probe_points.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('assets/img2_accuracy_across_positions_all_points.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('assets/img2_accuracy_heatmap.png')
    plt.close()

    print("Graphs have been saved in the assets directory.")

    # Generate CSV file
    with open('img2_validation_accuracy.csv', 'w', newline='') as csvfile:
    ...
    print("CSV file 'img2_validation_accuracy.csv' has been generated.")
    ...
    for point, results in all_results.items():
        if 'layer' in point:
            ...
            plt.savefig(f'assets/img2_ttt_heatmap_{point}.png')
            plt.close()
...
def main():
    ...
    model = load_model("out-img2-models/ckpt_iter_1100.pt").to(device)
    ...
    games = all_games[:3000]  # Sample 10000 games for processing
